...
def lambda_salt(prot_ionic, residues):
    # Create a new column name based on the salt concentration
    column_name = f"lambdas_{prot_ionic:.3f}"

    # Update the lambdas dynamically
    residues[column_name] = residues['lambdas'] + residues['alphas'] * (prot_ionic - 0.15)
    print(residues[column_name])
    return column_name
...
def calcEnergyMap(t,df,prot,rc):
    ...
    #salt_lambdas = lambda_salt(ionic,df)
    #lambdas = salt_lambdas
    salt_lambdas = lambda_salt(ionic,df)
    print('calcEnergyMap salt column:'+salt_lambdas)
    print(df.columns)
    sigmas = 0.5*(df.loc[pairs[:,0]].sigmas.values+df.loc[pairs[:,1]].sigmas.values)
    #lambdas = 0.5*(df.loc[pairs[:,0]].salt_lambdas].values+df.loc[pairs[:,1].salt_lambdas].values)
    lambdas = 0.5*(df.loc[pairs[:,0],salt_lambdas].values+df.loc[pairs[:,1],salt_lambdas].values)
...
def calcRgTensor(t,residues,seq,forces):
    ...
    prefactor = constants.Boltzmann*seq.temp/constants.hbar**2
    ...
    kT_over_hbar_omega = constants.Boltzmann*seq.temp*np.sqrt(prefactor/eigenvalues)
...
def gen_dcd(path):
    traj = md.load_dcd(path+'/traj.dcd', top=path+'/init_top.pdb') #changed so it loads top.pdb from above rather than init_top.pdb
    top = md.Topology()
    chain = top.add_chain()
    for res in traj.top.residues:
        residue = top.add_residue(res.name, chain)
        top.add_atom('CA', element=md.element.carbon, residue=residue)
    traj.top = top
    traj.xyz *= 10
    traj.unitcell_lengths *= 10
    traj.center_coordinates() #why are we doing centerDCD and also center_coordinates? and doing it twice?
    traj[-1].save_pdb(path+'/top.pdb')
    traj[10:].save_dcd(path+'/traj.dcd') #changed from save_xtc to save_dcd

def analyse(residues,path,seq): #deleted all the stuff that's in centerDCD above
    #if os.path.exists(path+'/traj.gsd'):
    #    traj = md.load(path+'/traj.gsd')
    #else:
    traj = md.load_dcd(path+'/traj.dcd',top=path+'/top.pdb')
    N_res = traj.n_atoms
    fasta = list(seq.fasta)
    #energy maps
...
def simulate(residues,sequences,seq_name,path):
    ...
    #salt_lambdas = lambda_salt(ionic_strength,residues)
    #lambdas = salt_lambdas
    # Adjust lambdas based on ionic strength
    lambda_column_name = lambda_salt(ionic_strength, residues)
    ...
    lambdamap = pd.DataFrame((residues[lambda_column_name].values+residues[lambda_column_name].values.reshape(-1, 1))/2,
            index=residues.lambdas.index,columns=residues.lambdas.index)
    ...
    hoomd.dump.gsd(filename=path+'/checkpoint.gsd', period=None, group=hoomd.group.all(), truncate=True, phase=0)

    hoomd.run(N_steps)
    gen_dcd(path)

residues = pd.read_csv('residues_CV3.csv').set_index('one',drop=False)
sequences = pd.read_csv('sox2.csv',index_col=0,delimiter=':')
t0 = time.time()
...
residues.to_csv('salt_residues.csv', index=True)
print('Timing Simulation & Analysis {:.3f}'.format(time.time()-t0))